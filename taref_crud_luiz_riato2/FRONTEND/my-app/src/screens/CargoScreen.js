...
    if (!cargo) return;
    ...
    // 9. Lógica para pegar o ID correto (Mongo usa _id, SQLite usa id_cargo)
    const idToDelete = cargo._id || cargo.id_cargo;

    hideDeleteDialog(); 
    try {
      // 10. Chama a função unificada
      const success = await deletarCargo(idToDelete);
      if (success) {
...
            <Ionicons name="person" size={16} color="#666" />
            {/* No Mongo, o 'populate' do backend retorna um objeto em 'id_funcionario_fk'.
              No SQLite, 'consultaCargosCompletos' retorna 'nome_funcionario'.
              Precisamos tratar ambos os casos.
            */}
            <Text style={styles.detailText}>
              {item.nome_funcionario || (item.id_funcionario_fk && item.id_funcionario_fk.nome) || 'N/A'}
            </Text>
          </View>
          <View style={styles.detailRow}>
            <Ionicons name="briefcase" size={16} color="#666" />
            <Text style={styles.detailText}>
              {item.nome_funcao || (item.id_funcao_fk && item.id_funcao_fk.nome_funcao) || 'N/A'}
            </Text>
          </View>
        </View>
      </View>
...
          keyExtractor={(item) => (item._id || item.id_cargo).toString()}
